...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const explanations = [
    ...
    "THE WAZE EPIPHANY: Before Waze, traffic apps used centralized data. Waze said: what if every driver becomes a sensor? Millions of real-time data points emerged. QIS says: what if every hospital becomes a research node? Every validated treatment outcome — distilled to 512 bytes, no patient data — routes to every other hospital managing similar patients. The network IS the intelligence. Waze gave us distributed traffic routing. QIS gives us distributed outcome routing. Same principle, different domain, same math.",
    ...
    "THE DISTILLATION STEP: This is what breaks the O(N squared) communication barrier. Instead of shipping raw datasets for pairwise comparison, each node compresses what it learned into a 512-byte outcome packet — an outcome delta, a confidence interval, a cohort count, a semantic fingerprint. The information density is maximized. The transmission cost is constant. A hospital managing 100,000 patients transmits the same 512-byte packet as a clinic with 3 patients. This is why QIS achieves O(log N) or better communication where naive approaches require O(N squared).",
