...
import { box, colorize, stripAnsi } from "consola/utils";
...
import { normalize, relative, resolve as resolve$1 } from "pathe";
...
import { parse as parse$1, render, walk } from "ultrahtml";
...
async function generateClient(options = {}, unlighthouse) {
	...
	if (prefix === "/") prefix = "";
	await cp(dirname(runtimeSettings.resolvedClientPath), runtimeSettings.generatedClientPath, { recursive: true });
  ...
  <\/script>`).replace(/(href|src)="\/assets\/(.*?)"/g, `$1="${prefix}assets/$2"`);
	...
	await writeFile(join(runtimeSettings.generatedClientPath, "assets", "payload.js"), `window.__unlighthouse_payload = ${JSON.stringify(staticData)}`, { encoding: "utf-8" });
...
function matches(pattern, path) {
	...
	while (p < patternLength) {
		if (pattern[p] === "$" && p + 1 === patternLength) return matchingLengths[numMatchingLengths - 1] === pathLength;
		if (pattern[p] === "*") {
			...
			for (let i = 0; i < numMatchingLengths; i++) {
				...
				if (matchLength < pathLength && path[matchLength] === pattern[p]) matchingLengths[numMatches++] = matchLength + 1;
			}
...
function createRoutes(options) {
	...
	files.forEach((file) => {
		...
		const route = {
			name: "",
			path: "",
			component: resolve$1(srcDir, file)
		};
		let parent = routes;
		keys.forEach((key, i) => {
...
async function discoverRouteDefinitions() {
	...
	const resolveFiles = async (dir) => {
		const { glob } = await import("tinyglobby");
		return await glob([
			join(dir, "**", `*.${supportedExtensions.length > 1 ? `{${supportedExtensions.join(",")}}` : supportedExtensions[0]}`),
			"!**/README.md",
			"!**/node_modules"
		], {
	...
	return createRoutes({
		...
		route.path = route.path.split("/").map((n) => {
			if (n.startsWith("[") && n.endsWith("]")) return `:${n.replace("[", "").replace("]", "").replace("...", "")}`;
			return n;
		}).join("/");
		return route;
	});
}
async function createApi(app) {
	const logger = useLogger();
	const { ws, resolvedConfig, runtimeSettings, hooks } = useUnlighthouse();
	const router = createRouter();
	router.get("/__lighthouse/", defineEventHandler((event) => sendRedirect(event, resolvedConfig.routerPrefix)));
	...
	router.get("/**", defineEventHandler(async (event) => {
		...
		const mimeType = mimeTypes[path.substring(path.lastIndexOf("."))];
		if ((await stat(join(runtimeSettings.generatedClientPath, path)).catch(() => null))?.isFile()) {
			if (mimeType) setResponseHeader(event, "Content-Type", mimeType);
			return serveStatic(event, {
				getContents: (id) => readFile(join(runtimeSettings.generatedClientPath, id)),
				getMeta: async (id) => {
					const s = await stat(join(runtimeSettings.generatedClientPath, id)).catch(() => null);
					if (!s?.isFile()) return;
					return {
						size: s.size,
						mtime: s.mtimeMs
...
function normaliseRoute(url) {
	...
	for (const matcher in resolvedConfig.scanner.customSampling) {
		if (!new RegExp(matcher).test(path)) continue;
...
const resolveReportableRoutes = async () => {
	...
	if (resolvedConfig.scanner.crawler) hooks.hook("discovered-internal-links", (path, internalLinks) => {
	...
	return map(groupBy(validUrls.map((url) => normaliseRoute(url)), resolvedConfig.client.groupRoutesKey.replace("route.", "")), (group) => {
...
function createFilter(options = {}) {
	...
	return function(path) {
		for (const v of [{
			...
			if (v.rules.filter((r) => r instanceof RegExp).some((r) => r.test(path))) return v.result;
			...
			if (stringRules.length > 0) {
				...
				if (toRouteMatcher(createRouter$1({
					routes,
					strictTrailingSlash: false
				})).matchAll(path).length > 0) return Boolean(v.result);
			}
		}
...
async function setupPage(page) {
	...
	browser.on("targetchanged", async (target) => {
		...
		if (page) {
			...
			if (resolvedConfig.extraHeaders) await page.setExtraHTTPHeaders(resolvedConfig.extraHeaders).catch(softErrorHandler("Failed to set extra headers"));
			if (resolvedConfig.userAgent) await page.setUserAgent(resolvedConfig.userAgent);
...
const extractHtmlPayload = async (page, route) => {
	...
	try {
		...
		return {
			success: true,
			payload: await (resolvedConfig.scanner.skipJavascript ? pageVisit.text() : page.evaluate(() => document.querySelector("*")?.outerHTML))
		};
	} catch (e) {
		return {
			success: false,
...
async function processHtml(html, site) {
	const seo = {
		favicon: "/favicon.ico",
		og: {},
		internalLinks: 0,
	...
	const internalLinks = [];
	const externalLinks = [];
	await walk(parse$1(html), async (node) => {
		if (node.type === 1) {
			if (node.name === "link") {
				...
				if (rel && rel.includes("icon") && href) seo.favicon = href;
			} else if (node.name === "meta") {
				const name = node.attributes?.name;
				const property = node.attributes?.property;
				const content = node.attributes?.content;
				if (name === "title" && content) seo.title = content;
				if (name === "description" && content) seo.description = content;
				if (property === "og:image" || name === "og:image") seo.og.image = content;
				if (property === "og:description" || name === "og:description") seo.og.description = content;
				if (property === "og:title" || name === "og:title") seo.og.title = content;
			} else if (node.name === "title") {
				if (!seo.title && node.children?.[0]?.type === 2) seo.title = node.children[0].value;
			} else if (node.name === "a") {
				const href = node.attributes?.href;
				if (href && !href.includes("javascript:") && !href.includes("mailto:") && !href.startsWith("#") && isImplicitOrExplicitHtml(href)) if (href.startsWith("/") && !href.startsWith("//") || href.includes(site)) internalLinks.push(href);
...
const inspectHtmlTask = async (props) => {
	...
	if (resolvedConfig.scanner.ignoreI18nPages && routeReport.seo.alternativeLangDefault && withoutTrailingSlash(routeReport.route.url) !== withoutTrailingSlash(routeReport.seo.alternativeLangDefault)) {
		...
		if (resolvedConfig.scanner.ignoreI18nPages) {
			...
			const baseMessage = `Page has a default alternative language ${routeReport.seo.alternativeLangDefaultHtml || ""}, ignoring \`${routeReport.route.path}\` and queueing \`${newRoute}\`.`;
	...
	if (resolvedConfig.cache) writeFileSync(htmlPayloadPath, html);
	return routeReport;
};
...
const runLighthouseTask = async (props) => {
	...
	if (resolvedConfig.cache && existsSync(reportJsonPath)) try {
		routeReport.report = normaliseLighthouseResult(routeReport, JSON.parse(readFileSync(reportJsonPath, "utf-8")));
		return routeReport;
	} catch (e) {
	...
	for (let i = 0; i < resolvedConfig.scanner.samples; i++) try {
		...
		if (await x(runtimeSettings.lighthouseProcessPath.endsWith(".ts") ? "jiti" : "node", [runtimeSettings.lighthouseProcessPath, ...args], {
			timeout: 360 * 1e3,
			nodeOptions: { stdio: [
				"pipe",
				"inherit",
	...
	if (report.audits?.["final-screenshot"]?.details?.data) await writeFile(join(routeReport.artifactPath, ReportArtifacts.screenshot), base64ToBuffer(report.audits["final-screenshot"].details.data));
	if (report.fullPageScreenshot?.screenshot.data) await writeFile(join(routeReport.artifactPath, ReportArtifacts.fullScreenScreenshot), base64ToBuffer(report.fullPageScreenshot.screenshot.data));
...
function createProgressBox() {
	...
	const update = (progressData) => {
		...
		if (clackSpinner && progressData.totalTasks > 0) {
			...
			if (progressData.averageScore !== void 0) message += ` • Score: ${formatScore(progressData.averageScore)}`;
			message += ` • ${formatTime(progressData.timeElapsed)}`;
			if (progressData.timeRemaining && progressData.timeRemaining > 0) message += ` • ETA: ${formatTime(progressData.timeRemaining)}`;
...
async function createUnlighthouseWorker(tasks) {
	...
	const taskCompletionTimes = /* @__PURE__ */ new Map();
	const startTime = Date.now();
	let currentTaskInfo = "";
	const monitor = () => {
		const timeDiff = Date.now() - cluster.startTime;
	...
	const updateProgressDisplay = () => {
		...
		const stats = monitor();
		const scoresFromReports = (routeReports.size > 0 ? [...routeReports.values()].filter((r) => Object.values(r.tasks).every((status) => status === "completed" || status === "ignore" || status === "failed")) : []).map((r) => r.report?.score).filter((score) => typeof score === "number");
	...
	const queueRoute = (route) => {
		...
		if (resolvedConfig.scanner.include || resolvedConfig.scanner.exclude) {
			if (!createFilter(resolvedConfig.scanner)(path)) {
		...
		if (resolvedConfig.scanner.dynamicSampling && resolvedConfig.scanner.dynamicSampling > 0) {
			...
			if ([...routeReports.values()].filter((r) => get(r, resolvedConfig.client.groupRoutesKey) === routeGroup).length >= resolvedConfig.scanner.dynamicSampling) return;
		}
		...
		const runTaskIndex = (idx = 0) => {
			...
			cluster.execute(routeReport, (arg) => {
				...
				if (!routeReport.tasksTime) routeReport.tasksTime = {};
				...
				const reportData = [`Time Taken: ${(ms / 1e3).toFixed(1)}s`];
	...
	const queueRoutes = (routes) => {
		...
		sortBy(routes, (r) => {
			...
			return r.path;
		}).forEach((route) => queueRoute(route));
	};
	const requeueReport = (report) => {
		...
		Object.values(ReportArtifacts).forEach((artifact) => {
			fs.rmSync(join(report.artifactPath, artifact), {
				force: true,
				recursive: true
			});
		});
...
const resolveUserConfig = async (userConfig) => {
	const logger = useLogger();
	const config = createDefu((obj, key, value) => {
	...
	if (config.root && config.discovery && config.discovery.pagesDir === "pages") {
		if (!existsSync(join(config.root, config.discovery.pagesDir))) {
	...
	if (process.getuid?.() === 0) {
		...
		if (!config.puppeteerOptions.args.includes("--no-sandbox")) config.puppeteerOptions.args.push("--no-sandbox");
		if (!config.puppeteerOptions.args.includes("--disable-setuid-sandbox")) config.puppeteerOptions.args.push("--disable-setuid-sandbox");
	}
...
function indentLines(string, spaces, firstLineSpaces) {
	...
	let s = "";
	if (lines.length) s = indent(firstLineSpaces === void 0 ? spaces : firstLineSpaces) + lines.shift();
	if (lines.length) {
...
function wrapAnsi(string, width) {
...
async function createUnlighthouse(userConfig, provider) {
	...
	if (userConfig.configFile && !isAbsolute(userConfig.configFile)) userConfig.configFile = join(process.cwd(), userConfig.configFile);
	...
	if (existsSync(mjsPath)) runtimeSettings.lighthouseProcessPath = mjsPath;
	else runtimeSettings.lighthouseProcessPath = join(runtimeSettings.moduleWorkingDir, "lighthouse.ts");
	runtimeSettings.configCacheKey = createHash("md5").update(JSON.stringify({
		...userConfig,
		version
	...
	ctx.setCiContext = async () => {
		...
		let outputPath = join(resolvedConfig.outputPath, $site.hostname.replace(":", "꞉"), runtimeSettings.configCacheKey || "");
		try {
	...
	ctx.setSiteUrl = async (url) => {
		...
		const outputPath = join(resolvedConfig.outputPath, runtimeSettings.siteUrl?.hostname.replace(":", "꞉") || "", runtimeSettings.configCacheKey || "");
...
async function fetchUrlRaw(url, resolvedConfig) {
	...
	if (resolvedConfig.cookies) headers.Cookie = resolvedConfig.cookies.map((cookie) => `${cookie.name}=${cookie.value}`).join("; ");
	...
	if (resolvedConfig.defaultQueryParams) {
		const u = new URL(url);
		for (const [k, v] of Object.entries(resolvedConfig.defaultQueryParams)) u.searchParams.set(k, String(v));
		finalUrl = u.toString();
	}
	while (attempt < maxRetries) {
		...
		try {
			...
			if (res.status < 200 || res.status >= 300 && !redirected) return {
				valid: false,
				redirected,
				response,
				redirectUrl
			...
			if (code) logger.error("Fetch error code:", code);
	...
	return {
		error: /* @__PURE__ */ new Error("Max retries reached"),
		valid: false
	};
}
