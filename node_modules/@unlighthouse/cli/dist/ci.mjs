import { a as createCli, i as handleError, n as validateHost, r as validateOptions, t as pickOptions } from "./_chunks/util.mjs";
...
import { getAncestorHash, getAuthor, getAvatarUrl, getCommitMessage, getCommitTime, getCurrentBranch, getCurrentHash, getExternalBuildUrl } from "@lhci/utils/src/build-context.js";
...
function reportCSVSimple(reports) {
	...
	return [headers.join(","), ...body.map((row) => row.join(","))].flat().join("\n");
}
function reportCSVExpanded(reports, { columns }) {
	const { headers, body } = csvSimpleFormat(reports);
	for (const k of Object.keys(columns)) {
		if (k === "overview") continue;
		if (!reports[0].report.categories.some((category) => category.key === k)) continue;
		headers.push(...columns[k].map((column) => ({
			column,
	...
	reports.forEach(({ report }, i) => {
		for (const k of Object.keys(columns)) {
			if (k === "overview") continue;
			if (!reports[0].report.categories.some((category) => category.key === k)) continue;
			body[i].push(...columns[k].map((column) => get(report, column.key.replace("report.", ""))).filter((val) => val?.scoreDisplayMode && val.scoreDisplayMode !== "informative" && val.scoreDisplayMode !== "notApplicable").map((val) => {
				if (val.scoreDisplayMode === "binary") return val.score;
				if (val.scoreDisplayMode === "numeric") return Math.round(val.numericValue * 100) / 100;
				return val.score;
			}));
		}
	});
...
function reportJsonExpanded(reports) {
	...
	const routes = reports.map((report) => {
		const categories = Object.values(report.report?.categories ?? {}).reduce((prev, category) => {
			metadata = {
				...metadata,
				categories: {
					...metadata.categories,
			...
			return {
				...prev,
				[category.key]: { score: category.score }
			};
		}, {});
	...
	return {
		summary: {
			score: Number.parseFloat((routes.reduce((prev, curr) => prev + curr.score, 0) / routes.length).toFixed(2)),
...
function extractCategoriesFromRoutes(routes) {
	...
	return Object.keys(categoriesWithAllScores).reduce((prev, key) => {
		const averageScore = Number.parseFloat((categoriesWithAllScores[key].scores.reduce((prev, curr) => prev + curr, 0) / categoriesWithAllScores[key].scores.length).toFixed(2));
...
function extractMetricsFromRoutes(routes) {
	...
	return Object.keys(metricsWithAllNumericValues).reduce((prev, key) => {
		const averageNumericValue = Number.parseFloat((metricsWithAllNumericValues[key].numericValues.reduce((prev, curr) => prev + curr, 0) / metricsWithAllNumericValues[key].numericValues.length).toFixed(2));
	...
	try {
		const api = new ApiClient({
			...
			basicAuth: typeof lhciAuth === "string" && lhciAuth.includes(":") ? {
				username: lhciAuth.split(":")[0],
				password: lhciAuth.split(":")[1]
			} : void 0
		});
		...
		for (const report of reports) {
			const lighthouseResult = JSON.parse(await readFile(`${report.artifactPath}/lighthouse.json`, "utf-8"));
			await api.createRun({
				projectId: project.id,
				buildId: build.id,
...
function generateReportPayload(reporter, _reports, config) {
	const reports = _reports.sort((a, b) => a.route.path.localeCompare(b.route.path)).filter((r) => {
		if (!r.report?.categories) return false;
		return r.report.audits;
	});
	if (reporter.startsWith("json")) {
		if (reporter === "jsonSimple" || reporter === "json") return reportJsonSimple(reports);
		if (reporter === "jsonExpanded") return reportJsonExpanded(reports);
	}
	if (reporter.startsWith("csv")) {
		if (reporter === "csvSimple" || reporter === "csv") return reportCSVSimple(reports);
		if (reporter === "csvExpanded") return reportCSVExpanded(reports, config);
	}
	if (reporter === "lighthouseServer") return reportLighthouseServer(reports, config);
...
async function run() {
	...
	await createUnlighthouse({
		...resolvedOptions,
		hooks: { "resolved-config": async (config) => {
	...
	if (!resolvedConfig.ci.reporter) resolvedConfig.ci.reporter = "jsonSimple";
	const logger = useLogger();
	let hasBudget = true;
	if (!resolvedConfig.ci?.budget) hasBudget = false;
	await setCiContext();
	...
	hooks.hook("worker-finished", async () => {
		const seconds = Math.round(((/* @__PURE__ */ new Date()).getTime() - startTime.getTime()) / 1e3);
		...
		if (hasBudget) {
			...
			worker.reports().forEach((report) => {
				...
				Object.values(categories).forEach((category) => {
					...
					if (!Number.isInteger(budget)) budget = resolvedConfig.ci.budget[category.key];
			...
			if (!hadError) logger.success("Score assertions have passed.");
		}
		if (resolvedConfig.ci.reporter) {
			...
			let path = "";
			if (payload) path = `\`./${relative(resolvedConfig.root, await outputReport(reporter, resolvedConfig, payload))}\``;
