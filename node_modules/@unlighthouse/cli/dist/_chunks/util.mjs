...
async function validateHost(resolvedConfig) {
	...
	if (site) {
		...
		if (!valid) {
			if (response?.status) logger.warn(`Request to site \`${site}\` returned an invalid http status code \`${response.status}\`. lease check the URL is valid and not blocking crawlers.`);
			else logger.warn(`Request to site \`${site}\` threw an unhandled exception. Please check the URL is valid and not blocking crawlers.`, error);
			logger.error("Site check failed. will attempt to proceed but may fail.");
		} else if (response) if (redirected && redirectUrl && !redirectUrl.includes(".")) {
		...
		} else logger.success(`Successfully connected to \`${site}\`. (Status: \`${response.status}\`).`);
	}
}
...
function validateOptions(resolvedOptions) {
	if (!resolvedOptions.site && resolvedOptions.urls?.length) resolvedOptions.site = resolvedOptions.urls[0];
	if (!resolvedOptions.configFile && !resolvedOptions.site) return handleError("Please provide a site to scan with --site <url>.");
	if (resolvedOptions.site && !isValidUrl(resolvedOptions.site)) return handleError("Please provide a valid site URL.");
	if (resolvedOptions?.ci?.reporter === "lighthouseServer") {
		if (!resolvedOptions?.ci?.reporterConfig?.lhciBuildToken) handleError("Please provide the lighthouse server build token with --lhci-build-token.");
		if (!resolvedOptions?.ci?.reporterConfig?.lhciHost) handleError("Please provide the lighthouse server build token with --lhci-host.");
	}
}
function pickOptions(options) {
	...
	picked.scanner = {};
	picked.urls = [];
	if (options.noCache) picked.cache = true;
	if (options.throttle) picked.scanner.throttle = true;
	if (options.sitemaps) {
	...
	if (options.enableJavascript) picked.scanner.skipJavascript = false;
	else if (options.disableJavascript) picked.scanner.skipJavascript = true;
	if (options.samples) picked.scanner.samples = options.samples;
	if (options.enableI18nPages) picked.scanner.ignoreI18nPages = false;
	else if (options.disableI18nPages) picked.scanner.ignoreI18nPages = true;
	if (options.desktop) picked.scanner.device = "desktop";
	else if (options.mobile) picked.scanner.device = "mobile";
	if (options.disableRobotsTxt) picked.scanner.robotsTxt = false;
	if (options.disableSitemap) picked.scanner.sitemap = false;
	if (options.urls) picked.urls = options.urls.split(",");
	if (options.excludeUrls) picked.scanner.exclude = options.excludeUrls.split(",");
	if (options.includeUrls) picked.scanner.include = options.includeUrls.split(",");
	if (options.disableDynamicSampling) picked.scanner.dynamicSampling = false;
	if (options.auth) {
	...
	if (options.cookies) picked.cookies = options.cookies.split(";").map(splitNameValue);
...
export { createCli as a, handleError as i, validateHost as n, validateOptions as r, pickOptions as t };
